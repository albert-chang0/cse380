...
\begin{document}
    ...
    \begin{center}
        \begin{tikzpicture}[node distance = 1.5cm, auto]
            ...
            \node[below of=small, node distance = 2cm, inner sep = 0pt] (branch1) {};
            \node[below of=branch1] (empty1) {};
            ...
            \node[left of = address, node distance = 3cm] (empty2) {};
            \node[below of = empty2, inner sep = 0pt] (branch2) {};
            \node[below of=branch2] (cont) {};
            \node[right of=branch2, node distance=7cm, inner sep = 0] (cont2) {};
            \path[line] (origin) -- (large);
            \path[line] (large) -- node [near start] {no} (small);
            \path[line] (small) -- node [near start] {no} (branch1);
            \draw (eparity) |- (branch1) -| (isolate);
            ...
            \draw (address) |- (branch2) -| (eparity);
            \draw (large) -- node [near start] {yes} +(7,0) -- (cont2);
            \draw (small) -- node [near start] {yes} +(7,0);
            \path[line, dashed] (cont2) -- +(0,-1.375);
            \path[line, dashed] (branch2) -- (cont);
        \end{tikzpicture}
    \end{center}
